...
namespace App\Controller\Admin\Orm;
    ...
    public function getResultsAsJson(): string
    {
        ...
        foreach ($this->getResults() ?? [] as $entityInstance) {
            ...
            $jsonResult['results'][] = [
                ...
                'entityMediaUrl' =>
                    $entityDto->getInstance() instanceof Media
                        ? $this->helper->asset($entityDto->getInstance(), 'imageFile150w')
                        : null,
            ];
        }

        $nextPageUrl = !$this->hasNextPage()
            ? null
